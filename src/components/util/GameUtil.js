 ...
 ** startingFunctionOfBoard: Custom function to create each element. This function
    must take an integer as unique parameter and return a valid React element.
 */
export default function generateGridNxN(
  className,
  size,
  startingFunctionOfBoard
) {
  var rows = [];
  for (let i = 0; i < size; i++) {
    let cols = [];
    for (let j = 0; j < size; j++) {
      cols.push(startingFunctionOfBoard(i * size + j));
    }
    rows.push(
